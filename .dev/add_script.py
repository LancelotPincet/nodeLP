...
def main() :
    ...
    while already_exists :
        ...
        script_path = path.parent / f'libsLP/nodeLP/src/nodelp/scripts/{name}_LP'
        module_path = path.parent / f'libsLP/nodeLP/src/nodelp/modules/{name}_LP'
    ...
    def copy_file(from_path, to_path) :
        ...
        string = string.replace('template_scriptlib', "nodeLP")
        string = string.replace('template_scriptlowerlib', "nodelp")
        string = string.replace('template_scriptequals', "="*len(name))
        string = string.replace('template_scrlibeq', "="*len("nodeLP"))
    ...
    toml_path = path.parent / f'libsLP/nodeLP/pyproject.toml'
    with open(toml_path, "r") as file:
        data = toml.load(file)
    data.setdefault("project", {}).setdefault("scripts", {})[name] = f"nodeLP.scripts.{name}_LP.{name}:{name}"
    ...
    scrjson['date'] = date
    json_path = path.parent / f'libsLP/nodeLP/src/nodelp/scripts.json'
    ...
    newrst_path = path.parent / f'libsLP/nodeLP/docs/source/{name}.rst'
    copy_file(rst_path, newrst_path)

    allscripts_path = path.parent / f'libsLP/nodeLP/docs/source/scripts.rst'
