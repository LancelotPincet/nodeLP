...
In this section, we will see how **nodeLP** can be installed in your Python projects.
...
Here, we want to create a new Python project with a dependency on **nodeLP**.
...
Then, we just need to add the dependencies to your project, typically dependency to **nodeLP**, with the command:

.. code-block:: shell

   uv add nodelp

You can redo this command for all the dependencies you might need by replacing the "nodeLP" by the other dependency name (the name ususally used in the command ``pip``).
...
.. tab-set::
   ...
   .. tab-item:: Anaconda tools (Spyder Jupyter)
      ...
      To be able to use another virtual environment, *Spyder* and *Jupyter* connects to specific kernels, respectively *spyder-kernels* and *ipykernel*, which are included in **nodeLP** library dependencies.
...
To finish, some routines may be provided with the library **nodeLP** :doc:`(see Reference Guide) <routines>`.
To use them, you can just download `routines source codes <https://github.com/LancelotPincet/nodeLP/tree/main/src/nodelp/routines>`_.
...
.. dropdown:: Using *Anaconda* distribution
   ...
   *Anaconda* is a Python distribution designed for scientific programming.
   ...
   Here we will explain how to setup up virtual environments dependent on **nodeLP** that you will be able to use on various projects.
   ...
   However some dependencies for this Python project might not be available in this *base* environment, like **nodeLP**.
   ...
   - create venv: ``conda create -n venv_nodeLP python``
   - activate venv: ``conda activate venv_nodeLP``
   - update pip: ``conda install pip``
   - add *Spyder* and *Jupyter* kernels: ``conda install spyder-kernels ipykernel``
   - install **nodeLP**: ``pip install nodelp``
...
.. dropdown:: Manual project
   ...
   6. Install **nodeLP** from terminal: ``pip install nodelp``
...
If ever you want to play with the library source code, to apply your own modifications, or even to contribute, you can access it on the `GitHub page <https://github.com/LancelotPincet/nodeLP>`_.
...
Now, you can clone in your current directory the `source code folder <https://github.com/LancelotPincet/nodeLP>`_ with the following command

.. code-block:: shell

   git clone https://github.com/LancelotPincet/nodeLP.git
